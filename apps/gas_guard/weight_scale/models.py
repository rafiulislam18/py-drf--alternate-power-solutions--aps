...
class ScaleDevice(models.Model):
    ...
    # The account this site belongs to; owners see it on their dashboard.
    ...
    # Cylinder profile — lets the API turn a raw scale weight into gas remaining.
    tare_kg = models.DecimalField(
        max_digits=6,
        decimal_places=2,
        default=Decimal('15.00'),
        help_text='Weight of the empty cylinder in kg.',
    )
    full_gas_kg = models.DecimalField(
        max_digits=6,
        decimal_places=2,
        default=Decimal('48.00'),
        help_text='Weight of the gas alone when the cylinder is full, in kg.',
    )
...
class WeightReading(models.Model):
    ...
    # WiFi signal strength reported by the device alongside the reading.

"""
Tests for PayFast ITN payment recording on the R199 solar-maintenance plan.

Mirrors the R99 plan's ITN tests: every COMPLETE notification writes an
immutable Payment row, and that row is the idempotency key that stops PayFast's
ITN retries from double-counting subscription months.
"""

from unittest.mock import patch

import pytest

from apps.request_solar_cleaning.models import Client, Payment, Subscription

NOTIFY_URL = '/request-solar-cleaning/payfast-notify/'


@pytest.fixture
def subscription(db):
    client = Client.objects.create(
        name='Sub One', email='subscriber@example.com', phone='0110000000'
    )
    return Subscription.objects.create(
        client=client,
        address='12 Main Rd, Cape Town',
        inverter_type='Sunsynk',
        inverter_size='5kW',
        installed_panels_count='12',
        is_active=False,
        subscription_length=0,
    )


def itn_payload(subscription, **overrides):
    payload = {
        'm_payment_id': str(subscription.id),
        'pf_payment_id': 'pf-solar-1',
        'payment_status': 'COMPLETE',
        'item_name': 'Monthly Subscription',
        'amount_gross': '199.00',
        'amount_fee': '6.90',
        'amount_net': '192.10',
        'token': 'tok-solar-123',
    }
    payload.update(overrides)
    return payload


@pytest.fixture(autouse=True)
def _no_network():
    with patch('apps.request_solar_cleaning.views.verify_payfast_ip', return_value=True), \
         patch('apps.request_solar_cleaning.views.EmailMessage'):
        yield


def post_itn(client, subscription, **overrides):
    return client.post(NOTIFY_URL, data=itn_payload(subscription, **overrides))


def test_complete_itn_records_payment(client, subscription):
    resp = post_itn(client, subscription)
    assert resp.status_code == 200

    payment = Payment.objects.get(pf_payment_id='pf-solar-1')
    assert payment.subscription == subscription
    assert payment.client == subscription.client
    assert str(payment.amount_gross) == '199.00'
    assert payment.payment_status == 'COMPLETE'
    assert payment.payfast_token == 'tok-solar-123'


def test_complete_itn_activates_and_counts_month(client, subscription):
    post_itn(client, subscription)

    subscription.refresh_from_db()
    assert subscription.is_active is True
    assert subscription.subscription_length == 1
    assert subscription.last_payment_date is not None


def test_duplicate_itn_records_one_payment(client, subscription):
    post_itn(client, subscription)
    resp = post_itn(client, subscription)

    assert resp.status_code == 200
    assert Payment.objects.filter(pf_payment_id='pf-solar-1').count() == 1


def test_duplicate_itn_does_not_double_count_months(client, subscription):
    post_itn(client, subscription)
    post_itn(client, subscription)

    subscription.refresh_from_db()
    assert subscription.subscription_length == 1


def test_distinct_payments_each_recorded(client, subscription):
    post_itn(client, subscription, pf_payment_id='pf-solar-month-1')
    post_itn(client, subscription, pf_payment_id='pf-solar-month-2')

    assert Payment.objects.count() == 2

    subscription.refresh_from_db()
    assert subscription.subscription_length == 2


def test_wrong_amount_records_nothing(client, subscription):
    resp = post_itn(client, subscription, amount_gross='99.00')

    assert resp.status_code == 400
    assert Payment.objects.count() == 0

    subscription.refresh_from_db()
    assert subscription.is_active is False


def test_non_complete_status_records_nothing(client, subscription):
    resp = post_itn(client, subscription, payment_status='FAILED')

    assert resp.status_code == 200
    assert Payment.objects.count() == 0
